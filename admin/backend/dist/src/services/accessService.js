...
exports.ensureOwnerOrAdmin = ensureOwnerOrAdmin;
exports.hasAccess = hasAccess;
exports.grantAccess = grantAccess;
exports.revokeAccess = revokeAccess;
const ApiError_1 = __importDefault(require("../utils/ApiError"));
const AccessControl_1 = __importDefault(require("../models/AccessControl"));
...
async function logAccess({ evidenceId, accessedBy, action, req, status = 'allowed', details = {} }) {
    await AccessLog_1.default.create({
        evidenceId,
        accessedBy,
        action,
        status,
...
async function hasAccess(evidence, user, action = 'view') {
    ...
    const grant = await AccessControl_1.default.findOne({
...
async function grantAccess({ evidenceId, grantedTo, permissions, grantedBy, req }) {
    ...
    const record = await AccessControl_1.default.findOneAndUpdate({ evidenceId, grantedTo }, {
        evidenceId,
        grantedTo,
        ...
        status: 'active',
        revokedAt: null
    }, { upsert: true, new: true, setDefaultsOnInsert: true }).populate('evidenceId grantedTo grantedBy');
    await logAccess({
        evidenceId,
...
async function revokeAccess({ evidenceId, grantedTo, revokedBy, req }) {
    ...
    const record = await AccessControl_1.default.findOneAndUpdate({ evidenceId, grantedTo, status: 'active' }, { status: 'revoked', revokedAt: new Date() }, { new: true });
    if (!record) {
        throw new ApiError_1.default(404, 'Active access grant not found');
    }
    await logAccess({
        evidenceId,
