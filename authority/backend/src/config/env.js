...
module.exports = {
  ...
  encryptFilesBeforeUpload: env.ENCRYPT_FILES_BEFORE_UPLOAD,
  encryptionKey: parseFileEncryptionKey(env.FILE_ENCRYPTION_KEY),
  cloudinary: {
  ...
  maxFileSizeBytes: Number(env.MAX_FILE_SIZE_MB) * 1024 * 1024,
  rateLimitWindowMs: Number(env.RATE_LIMIT_WINDOW_MS),
  rateLimitMax: Number(env.RATE_LIMIT_MAX),
  emergencyBatchSeconds: Number(env.EMERGENCY_BATCH_SECONDS),
  travelBatchSeconds: Number(env.TRAVEL_BATCH_SECONDS),
