...
const { uploadEncryptedBuffer, downloadBufferFromUrl, deleteStoredFile } = require('./storageService');
const { bufferHash, buildEvidenceHash } = require('./hashService');
const blockchainService = require('./blockchainService');
const { enqueueBlockchainWrite } = require('./queueService');
const { hasAccess, logAccess } = require('./accessService');
const { isConfigured: blockchainConfigured } = require('./blockchainService');
const { emergencyBatchSeconds, travelBatchSeconds, encryptFilesBeforeUpload } = require('../config/env');
...
async function verifyEvidenceById({ evidenceId, user, req }) {
  ...
  const isBlockchainConfigured = blockchainConfigured();
  let chainMatch = null;
  let chainStatus = 'NOT_CONFIGURED';

  if (isBlockchainConfigured) {
    try {
      chainMatch = await blockchainService.verifyEvidenceHash(evidence.hash);
      chainStatus = chainMatch ? 'VERIFIED' : 'MISMATCH';
    } catch (error) {
      chainMatch = null;
