...
exports.updateAuthorityStatus = updateAuthorityStatus;
exports.exportAdminUsers = exportAdminUsers;
...
const AdminReport_1 = __importDefault(require("../models/AdminReport"));
...
async function updateAuthorityStatus({ userId, status }) {
    ...
    if (user.role !== 'authority') {
        throw new ApiError_1.default(400, 'Only authority accounts can be managed from this endpoint');
    }
    user.status = status;
    await user.save();
...
async function evaluateSecurityControls({ notes = '', reviewedBy }) {
    const [totalUsers, pendingUsers, suspendedUsers, failedEvidence, failedRetryEvidence, pendingEvidence] = await Promise.all([
        ...
        User_1.default.countDocuments({ status: 'suspended' }),
        Evidence_1.default.countDocuments({ blockchainStatus: 'FAILED' }),
        Evidence_1.default.countDocuments({ blockchainStatus: 'FAILED_RETRY' }),
        Evidence_1.default.countDocuments({ blockchainStatus: 'PENDING' })
    ]);
    ...
    const review = await AdminReport_1.default.create({
...
async function generateAuditReport({ from, to, format = 'json', requestedBy }) {
    ...
    const [totalUsers, activeUsers, pendingUsers, suspendedUsers, totalEvidence, verifiedEvidence, evidencePending, evidenceFailed, totalAccessEvents, storageAgg] = await Promise.all([
        ...
        User_1.default.countDocuments({ status: 'pending' }),
        User_1.default.countDocuments({ status: 'suspended' }),
        Evidence_1.default.countDocuments(evidenceFilter),
        Evidence_1.default.countDocuments({ ...evidenceFilter, blockchainStatus: 'ON_CHAIN' }),
        Evidence_1.default.countDocuments({ ...evidenceFilter, blockchainStatus: 'PENDING' }),
        Evidence_1.default.countDocuments({
            ...evidenceFilter,
            blockchainStatus: { $in: ['FAILED', 'FAILED_RETRY'] }
        }),
        AccessLog_1.default.countDocuments(accessFilter),
        Evidence_1.default.aggregate([
    ...
    const report = await AdminReport_1.default.create({
        reportType: 'audit',
...
async function getAdminSettings() {
    const [failedEvidence, failedRetryEvidence, pendingEvidence, latestReview, storageAgg] = await Promise.all([
        ...
        Evidence_1.default.countDocuments({ blockchainStatus: 'FAILED_RETRY' }),
        Evidence_1.default.countDocuments({ blockchainStatus: 'PENDING' }),
        AdminReport_1.default.findOne({ reportType: 'security_review' }).sort({ createdAt: -1 }).lean(),
        Evidence_1.default.aggregate([
            { $unwind: { path: '$files', preserveNullAndEmptyArrays: true } },
            { $group: { _id: null, totalBytes: { $sum: { $ifNull: ['$files.size', 0] } } } }
        ])
...
async function getProjectActivitySummary() {
    ...
    const [totalAuthorities, totalEvidence, verifiedEvidence, pendingEvidence, flaggedEvidence, recentEvidence, recentAccessLogs] = await Promise.all([
        User_1.default.countDocuments({ role: { $in: ['admin', 'authority'] } }),
        Evidence_1.default.countDocuments(),
        Evidence_1.default.countDocuments({ blockchainStatus: 'ON_CHAIN' }),
        Evidence_1.default.countDocuments({ blockchainStatus: 'PENDING' }),
        Evidence_1.default.countDocuments({ blockchainStatus: { $in: ['FAILED', 'FAILED_RETRY'] } }),
        Evidence_1.default.find({ timestamp: { $gte: fourteenDaysAgo } })
            .sort({ timestamp: -1 })
            .populate('ownerUserId', 'name role')
            .lean(),
        AccessLog_1.default.find({ createdAt: { $gte: fourteenDaysAgo } })
            .sort({ createdAt: -1 })
